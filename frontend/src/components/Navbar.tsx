...
const Navbar = () => {
  ...
  const { cityName, residential,locationLoading, permissionDenied } = useUserLocation();

  const handleLogout = async () => {
    try { await authService.post("/api/auth/logout"); } catch (_) {}
    queryClient.clear();
  ...
  return (
    <>
      ...
      >
        {/* ── top row ── */}
        <div className="flex items-center justify-between px-6 md:px-10" style={{ height: 64 }}>

          {/* logo */}
          <NavLink to="/" className="flex items-center gap-2 select-none flex-shrink-0" style={{ textDecoration: "none" }}>
            <span className="w-9 h-9 rounded-xl flex items-center justify-center text-lg"
              style={{ background: "linear-gradient(135deg,#E23774,#FF6B35)" }}>🍅</span>
            <span className="text-3xl tracking-widest"
              style={{ fontFamily: "'Bebas Neue', sans-serif", color: "#E23774" }}>Tomato</span>
          </NavLink>
              ...
              <li key={link.to}>
                <NavLink to={link.to} end
                  ...
                  style={({ isActive }) => ({
                    ...
                    background: isActive ? "rgba(226,55,116,0.08)" : "transparent",
                    textDecoration: "none",
                  })}>
                  {link.label}
                </NavLink>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button
              ...
              style={{
                ...
                color: "white", boxShadow: "0 4px 16px rgba(226,55,116,0.25)",
                border: "none", cursor: "pointer",
              }}
              onMouseEnter={e => (e.currentTarget.style.boxShadow = "0 6px 24px rgba(226,55,116,0.4)")}
              onMouseLeave={e => (e.currentTarget.style.boxShadow = "0 4px 16px rgba(226,55,116,0.25)")}
            >
              <FiShoppingCart size={16} />
              <span>Cart</span>
              <span className="w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold"
                style={{ background: "rgba(255,255,255,0.3)" }}>0</span>
            </button>
            ...
            {userData ? (
              <div className="relative" style={{ zIndex: 100 }}>
                ...
                >
                  ...
                  <span className="hidden md:block text-sm font-medium pr-1"
                    style={{ color: "#1A0A00", maxWidth: 90, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {userData.name?.split(" ")[0]}
                  </span>
                  <FiChevronDown size={13} color="#E23774"
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-52 rounded-2xl overflow-hidden"
                      style={{
                        ...
                        boxShadow: "0 16px 48px rgba(226,55,116,0.18), 0 2px 8px rgba(0,0,0,0.08)",
                        border: "1.5px solid rgba(226,55,116,0.12)",
                        animation: "dropIn 0.18s cubic-bezier(0.34,1.56,0.64,1) both",
                      }}>
                      <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(226,55,116,0.08)" }}>
                        <p className="text-sm font-semibold" style={{ color: "#1A0A00" }}>{userData.name}</p>
                        <p className="text-xs mt-0.5 truncate" style={{ color: "#aaa" }}>{userData.email}</p>
                        {userData.role && (
                          <span className="mt-1.5 inline-block text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"
                            style={{ background: "rgba(226,55,116,0.1)", color: "#E23774" }}>
                            {userData.role}
                          </span>
                        )}
                      </div>
                      {[{ icon: "👤", label: "Profile" }, { icon: "📦", label: "My Orders" }, { icon: "⚙️", label: "Settings" }].map((item) => (
                      ...
                      <div style={{ borderTop: "1.5px solid rgba(226,55,116,0.08)" }}>
                        <button onClick={handleLogout}
                          className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-left"
                          style={{ color: "#E23774", background: "transparent", border: "none", cursor: "pointer" }}
                          onMouseEnter={e => (e.currentTarget.style.background = "rgba(226,55,116,0.06)")}
                          onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                          🚪 Logout
                        </button>
                      </div>
              ...
              <NavLink to="/login" className="px-5 py-2 rounded-2xl text-sm font-semibold"
                style={{
                  background: "linear-gradient(135deg,#E23774,#FF6B35)",
            ...
            <button
              ...
              style={{ background: "rgba(226,55,116,0.08)", border: "none", cursor: "pointer" }}
              onClick={() => setMobileOpen((p) => !p)}
            >
              {[0, 1, 2].map((i) => (
                <span key={i} className="block rounded-full transition-all duration-300"
                  style={{
                    width: 20, height: 2, background: "#E23774",
                    transform:
        ...
        {/* ── search bar row ── */}
        <div className="px-6 md:px-10 pb-3 pt-1"
          ...
          <div className="flex items-center gap-3 max-w-2xl mx-auto">
            ...
            <button
              ...
              style={{
                background: permissionDenied ? "rgba(200,200,200,0.1)" : "rgba(226,55,116,0.07)",
                color: permissionDenied ? "#aaa" : "#E23774",
                border: `1.5px solid ${permissionDenied ? "rgba(200,200,200,0.3)" : "rgba(226,55,116,0.15)"}`,
                cursor: "pointer",
              }}
              onMouseEnter={e => !permissionDenied && (e.currentTarget.style.background = "rgba(226,55,116,0.13)")}
              onMouseLeave={e => !permissionDenied && (e.currentTarget.style.background = "rgba(226,55,116,0.07)")}
            >
              <FiMapPin size={14} />
            ...
            <div className="flex-1 relative flex items-center">
              <FiSearch size={16} className="absolute left-3.5 pointer-events-none" style={{ color: "#ccc" }} />
              <input
                ...
                style={{
                  paddingLeft: 38, paddingRight: search ? 84 : 16,
                  paddingTop: 10, paddingBottom: 10,
                  background: "white", border: "1.5px solid rgba(226,55,116,0.15)",
                  color: "#1A0A00", boxShadow: "0 2px 8px rgba(226,55,116,0.05)",
                  fontFamily: "'DM Sans', sans-serif",
                }}
                onFocus={e => (e.currentTarget.style.borderColor = "#E23774")}
                onBlur={e => (e.currentTarget.style.borderColor = "rgba(226,55,116,0.15)")}
              />
              {search && (
                <button className="absolute right-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-white"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setMobileOpen(false)}>
          <div className="absolute left-0 right-0 mx-4 rounded-2xl overflow-hidden"
            style={{
              top: 64, background: "white",
              ...
              {navLinks.map((link) => (
                <li key={link.to}>
                  <NavLink to={link.to} end onClick={() => setMobileOpen(false)}
                    ...
                    style={({ isActive }) => ({
                      color: isActive ? "#E23774" : "#555",
                      background: isActive ? "rgba(226,55,116,0.06)" : "transparent",
                      textDecoration: "none",
                    })}>
                    {link.label}
                  </NavLink>
            ...
            <div style={{ borderTop: "1.5px solid rgba(226,55,116,0.08)" }} className="px-5 py-3">
              <button onClick={handleLogout}
